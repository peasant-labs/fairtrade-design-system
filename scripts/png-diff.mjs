/* shared pixel-diff primitive — ONE implementation behind BOTH the pre/post parity gate
   (imgdiff.mjs) and the graph node-visual theme oracle (graph-oracle.mjs). There is no
   second copy of the diff loop, so the two gates can never disagree on what "a pixel
   differs" means.

   diffPixels decodes two PNG data: URLs inside a headless page (data: images don't taint
   the canvas, so getImageData works with no extra dependency), draws both onto a canvas,
   and counts the pixels whose per-channel delta exceeds `tol`.

   `tol` semantics: tol=16 means "no channel differs by more than 16/255"; a pixel is
   counted as differing only when at least one of R/G/B exceeds the threshold. This
   tolerates sub-pixel anti-aliasing shimmer from font rendering, so the count reflects
   real colour change, not AA noise. A return of `diff: 0` means no pixel exceeded `tol`. */
