...
try {
  for (const theme of ['dark', 'light']) {
    ...
    for (const fixture of fixtures.cases) {
      ...
      assert.equal(styles.children.paddingLeft, '24px', 'one indent step per nested list')
      // One step: the chip and the member rows sit one --sp-5 in from the owner.
      assert.ok(Math.abs(styles.indent.control - styles.indent.owner - 24) < 0.5, 'chip is indented one step under its owner row')
      assert.ok(Math.abs(styles.indent.member - styles.indent.owner - 24) < 0.5, 'member rows sit one step in from the owner row')
      if (geometry.anchors.length > 0) {
