...
async function runMountedCase({ browser, origin, name, testCase, geometry, sourceMarker }) {
  ...
  try {
    ...
    requireCase(observed.retained.styles.maxWidth === geometry.retainedMaxWidth && Math.abs(observed.retained.width - observed.parent.contentWidth) <= 0.5, name, 'retained semantic host width', { styles: observed.retained.styles, width: observed.retained.width, parentContentWidth: observed.parent.contentWidth }, `${geometry.retainedMaxWidth} and full parent content width`, 'retain the explicit .txn-center max-width reset and full parent-content width')
    requireCase(observed.retained.styles.marginLeft === geometry.retainedMarginLeft && observed.retained.styles.marginRight === geometry.retainedMarginRight, name, 'retained semantic host margins', observed.retained.styles, `${geometry.retainedMarginLeft}/${geometry.retainedMarginRight}`, 'retain the explicit .txn-center horizontal margin reset')
    requireCase(observed.retained.styles.paddingLeft === geometry.retainedPaddingLeft && observed.retained.styles.paddingRight === geometry.retainedPaddingRight, name, 'retained semantic host padding', observed.retained.styles, `${geometry.retainedPaddingLeft}/${geometry.retainedPaddingRight}`, 'retain the explicit .txn-center inline padding reset')
