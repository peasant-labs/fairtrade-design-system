...
- **conversation window** (`48-conversation.jsx`) the transcript reading view: a header with breadcrumb + title + meta that leaves once the full trace is scrolled (the pinned tab strip, the condensed scrubber header with provider + model + position, and the turns bar stay as the persistent navigation; scrolling back to the top restores it), role-accented turns (user = teal, assistant = amber, each led by an icon), collapsible tool-call rows with tool icons, thinking blocks, unified diff (rail + gutter + sign), code blocks, a persistent footer action bar.
