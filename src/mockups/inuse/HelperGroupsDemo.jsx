...
// The demo owns its fixture paging and selection just as a host owns its API.
// Production components never import these examples or parse their data.
const fixtures = YAML.parse(fixtureSource)

export default function HelperGroupsDemo({ scenario = 'three-independent-counts' }) {
  ...
  const renderRow = (id) => <HelperThreadRow {...fixtures.rows[id]} {...(fixture.update?.id === id ? fixture.update : {})}
    ...
    selected={selected.includes(id)} onSelect={onSelect}>
  ...
  return <div className="helper-demo" data-helper-demo={scenario}>
    <div hidden={opened !== null}>
      ...
      {fixture.groups.map((group) => {
        ...
        return <HelperGroupListItem key={group.id}
          owner={fixture.owner ? renderRow(fixture.owner) : undefined} ownerStatus={fixture.ownerStatus}>
          <HelperGroup groupId={group.id} memberScope={refreshed ? `${group.scope}-refreshed` : group.scope} helperThreadCount={group.count}
            members={group.pages[state.page - 1]} renderMember={renderRow} getMemberKey={(id) => id}
            page={state.page} limit={group.pages[0].length} total={group.count} status={state.status}
            onRequestPage={(request) => {
              setLastRequest(request)
              setPages((previous) => ({ ...previous, [group.id]: { page: request.page,
                status: refreshed ? 'ready' : fixture.status || 'ready' } }))
            }}
            onRefreshList={() => {
              setRefreshed(true)
              setPages({})
      ...
      {lastRequest && <p className="helper-demo-summary">requested helper page {lastRequest.page}</p>}
    </div>
    {opened !== null && <div>
