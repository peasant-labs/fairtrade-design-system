...
- **`wire-types.js`** — the canonical wire shapes the Go backend sends over the local
  `session_detail` channel (grounded in `schema/develop/local_api.go`). The SOLE input the adapter
  touches; no component binds a wire type directly.
...
`adapter.js` (`adaptTranscript`) is the only code that reads the wire and the only drift-absorption
boundary. Both data modules carry `// @ts-check` and are pinned by `tsc -p tsconfig.contract.json`
(`pnpm test:contract`).
...
| `RENDER_WHEN_PRESENT` | fed by fixtures / the hand-ported TS shape today; the real source is one of the above once surfaced. Absent ⇒ the field is simply not rendered. |
...
populated from fixtures / the hand-ported shape; when the backend surfaces commits on the wire, the
...
| `session.git.author` | `gitContext.user` (drifted TS `SessionGitContext.user`); absent on the Go-flat wire | `RENDER_WHEN_PRESENT` | IMPLEMENTED (`cookGit` binds `gitContext.user`; drifted shape / fixture only) |
...
| `commit.adds` / `dels` / `files` (per-commit churn) | `gitops.DiffStats` per commit — not yet joined to session commits | `BACKEND_SMALL_EXT` + `RENDER_WHEN_PRESENT` | IMPLEMENTED (fixture / hand-ported shape); SURFACE = ingest ext + columns + migration |
