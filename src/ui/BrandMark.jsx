/* BrandMark - the single source of truth for provider/company logos in the component
   ...
   Path geometry originates from Simple Icons (https://simpleicons.org, CC0). It is INLINED
   here (not referenced via <use href="#b-*">) so the component is self-contained and renders
   in Storybook and in any app without the document-global <symbol> sheet. Marks recolor via
   currentColor (single-color theming only) and re-theme for free under [data-theme]. They are
   kept small, single-color, undistorted, and paired with the provider's name - nominative use,
   never implying endorsement. Trademarks remain their owners'; CC0 covers the geometry, not the
   marks' trademark rights.

   a11y: a mark next to a visible text label is decorative (default: aria-hidden, focusable=false).
   A mark that stands alone AS the identity must pass `label` to become informative
   (role="img" + aria-label). Always focusable=false so it never tab-traps in IE/Edge legacy. */

/* viewBox is 0 0 24 24 for every mark; the wheat logo (#logo) is a separate brand glyph and
   intentionally not here. opencode's mark is provisional upstream (permissions in review). */
const PATHS = {
  claude:
...
/* human names for the informative (label-bearing) case, so a standalone mark announces the
   company rather than a slug. */
const BRAND_NAME = {
  claude: 'Claude',
  gemini: 'Gemini',
  openai: 'OpenAI',
...
export function resolveBrand(name) {
  ...
  return PATHS[canonical] ? canonical : null
}
...
export default function BrandMark({ name, size, label, className = '', ...rest }) {
  ...
  const sizeProps = size != null ? { width: size, height: size, style: { width: size, height: size } } : {}
  const a11y = label
    ? { role: 'img', 'aria-label': label === true ? BRAND_NAME[key] : label, focusable: 'false' }
    : { 'aria-hidden': 'true', focusable: 'false' }
  return (
    <svg className={classes} viewBox="0 0 24 24" {...sizeProps} {...a11y} {...rest}>
      <path d={PATHS[key]} />
    </svg>
  )
}
