/* BrandMark - the single source of truth for provider/company logos in the component
   ...
   single-color, undistorted, and paired with the provider's name, never implying endorsement.
...
/* Simple Icons marks use a 0 0 24 24 viewBox and fill most of it. Strike's path is its official
   geometry byte-for-byte, but that geometry occupies only a 10x20 area of its source 32x32 canvas,
   which would render the bolt at roughly half its peers' optical weight. The viewBox below is a
   crop window centred on the glyph (5 units clearance each side, exact height fit) so the mark
   matches the others without altering a single path coordinate.
   The wheat logo (#logo) is a separate brand glyph and intentionally not here. opencode's mark
   is provisional upstream (permissions in review). */
const PATHS = {
  claude:
 ...
 *
 * @param {object} props
 * @param {'claude'|'gemini'|'openai'|'cursor'|'opencode'|'strike'|'anthropic'|'google'|'codex'|string} props.name
