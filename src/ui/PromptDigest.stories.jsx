...
const shortChain = {
  ...
  skills: [
    ...
    { name: '/superpowers:writing-plans', invocationCount: 1 },
  ...
  items: [
    ...
    { kind: 'skill', transcriptId: TRANSCRIPT_A, timestamp: '2026-09-06T14:02:30Z', text: '/superpowers:brainstorming', turnIndex: 5 },
    ...
    { kind: 'skill', transcriptId: TRANSCRIPT_B, timestamp: '2026-09-07T09:16:20Z', text: '/superpowers:writing-plans', turnIndex: 3 },
...
const collapsedChain = {
  ...
  skills: [{ name: '/superpowers:brainstorming', invocationCount: 1 }],
  items: [
...
export const Chain = {
  ...
  play: async ({ canvasElement }) => {
    ...
    const invocations = canvas.getAllByText('/superpowers:brainstorming')
    ...
    // a commit anchor shows the abbreviated sha and leaves for GitHub
    const sha = canvas.getByText(SHA_ONE.slice(0, 7))
...
export const CollapsedBoundaries = {
  ...
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement)
    // every session keeps a visible boundary, and a collapsed one states the run it stands in for
